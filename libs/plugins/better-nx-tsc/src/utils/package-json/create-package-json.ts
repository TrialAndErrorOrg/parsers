...
export function createPackageJson(
  ...
  if (existsSync(`${graph.nodes[projectName].data.root}/package.json`)) {
    try {
      packageJson = readJsonFile(`${graph.nodes[projectName].data.root}/package.json`)
  ...
  Object.entries(npmDeps.peerDependencies).forEach(([packageName, version]) => {
    if (!packageJson.peerDependencies?.[packageName]) {
      ...
      if (!isOptionalPeer) {
        if (!options.isProduction || rootPackageJson.dependencies?.[packageName]) {
  ...
  packageJson.peerDependenciesMeta &&= sortObjectByKeys(packageJson.peerDependenciesMeta)
