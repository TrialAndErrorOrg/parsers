...
jest.doMock('nx/src/utils/workspace-root', () => {
  return {
    workspaceRoot: '/root',
  }
})
...
jest.mock('nx/src/utils/typescript')

import { vol } from 'memfs'
import tsUtils = require('nx/src/utils/typescript')
import { expandNamedInput, filterUsingGlobPatterns, Hash, Hasher } from './hasher.js'
...
describe('Hasher', () => {
  ...
  it('should be able to handle multiple filesets per project', async () => {
    ...
    expect(parentHash.details.nodes['{workspaceRoot}/global1']).toEqual('global1.hash')
    expect(parentHash.details.nodes['{workspaceRoot}/global2']).toBe('global2.hash')
    expect(parentHash.details.nodes['env:MY_TEST_HASH_ENV']).toEqual('MY_TEST_HASH_ENV_VALUE')
    ...
    expect(childHash.details.nodes['{workspaceRoot}/global1']).toEqual('global1.hash')
  ...
  it('should throw an error when failed to execute runtimeCacheInputs', async () => {
    ...
    try {
      ...
      expect(e.message.includes(' not found') || e.message.includes('not recognized')).toBeTruthy()
    }
  })
    ...
    const hasher = new Hasher(
      {
        ...
        dependencies: {
          'npm:react': [],
          app: [{ source: 'app', target: 'npm:react', type: DependencyType.static }],
        },
        allWorkspaceFiles,
      },
      {} as any,
  ...
  describe('filterUsingGlobPatterns', () => {
    ...
    it('should OR all positive patterns and AND all negative patterns (when positive patterns)', () => {
      ...
      expect(filtered.map((f) => f.file)).toEqual(['root/a.ts', 'root/b.js', 'root/c.spec.ts'])
    })

    it('should handle projects with the root set to .', () => {
      const filtered = filterUsingGlobPatterns(
        '.',
        [{ file: 'a.ts' }, { file: 'b.md' }, { file: 'dir/a.ts' }, { file: 'dir/b.md' }] as any,
