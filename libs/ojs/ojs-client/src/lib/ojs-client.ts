...
export default class OJS {
  ...
  constructor({ endpoint = DEFAULT_ENDPOINT, token = '', version = '1.0.0', cookie = '' } = {}) {
    ...
    this.authenticatedFetch = async (method, path, data, query = {}) => {
      ...
      const queryString = `?${new URLSearchParams(queryWithApiToken).toString()}`
  ...
  post(path: string, data: Record<string, any>, query: Record<string, any> = {}) {
    return this.authenticatedFetch('POST', path, data, query)
  }

  put(path: string, data: Record<string, any>, query: Record<string, any> = {}) {
   ...
   */
  async submission(id: number): Promise<Responses<'/submissions/{submissionId}', 'get'>> {
   ...
   */
  async deleteSubmission(id: number): Promise<Responses<'/submissions/{submissionId}', 'delete'>> {
  ...
  async postPublication(
    ...
    const submission = await this.post(`/submissions/${submissionId}/publications`, body)
  ...
  async publications(
    ...
    const submission = await this.get(`/submissions/${submissionId}/publications`)
  ...
  ): Promise<Responses<'/submissions/{submissionId}/publications/{publicationId}', 'get'>> {
    const submission = await this.get(`/submissions/${submissionId}/publications/${publicationId}`)
  ...
  async editPublication(
    ...
    publicationId: number,
    body: paths['/submissions/{submissionId}/publications/{publicationId}']['put']['parameters']['body']['schema'],
  ): Promise<Responses<'/submissions/{submissionId}/publications/{publicationId}', 'put'>> {
  ...
  ): Promise<Responses<'/submissions/{submissionId}/files/{submissionFileId}', 'get'>> {
    return await this.get(`/submissions/${submissionId}/files/${submissionFileId}`, query)
  }
  ...
  async editFile(
    ...
    return await this.put(`/submissions/${submissionId}/files/${submissionFileId}`, body, query)
  }
  ...
  async deleteFile(
    ...
    return await this.delete(`/submissions/${submissionId}/files/${submissionFileId}`, query)
  }
