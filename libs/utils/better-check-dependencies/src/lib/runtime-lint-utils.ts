...
export function hasNoneOfTheseTags(proj: ProjectGraphProjectNode, tags: string[]): boolean {
...
export function findDependenciesWithTags(
  ...
  return allReachableProjects.map((project) =>
    targetProject.name === project ? [targetProject] : getPath(graph, targetProject.name, project),
  )
}
...
export function getTargetProjectBasedOnRelativeImport(
  ...
  const targetFile = normalizePath(path.resolve(sourceDir, imp)).substring(projectPath.length + 1)
...
export function findProject(
  ...
  return projectGraph.nodes[findProjectForPath(sourceFilePath, projectRootMappings)]
}
...
export function onlyLoadChildren(
  ...
  return (
    (graph.dependencies[sourceProjectName] || []).filter((d) => {
      ...
      return onlyLoadChildren(graph, d.target, targetProjectName, [...visited, sourceProjectName])
    }).length > 0
  )
}
...
function isConstraintBanningProject(
  ...
  if (
    bannedExternalImports?.some((importDefinition) => mapGlobToRegExp(importDefinition).test(imp))
  ) {
    return true
  }
...
export function hasBannedDependencies(
  ...
  depConstraint: DepConstraint,
  imp: string,
): Array<[ProjectGraphExternalNode, ProjectGraphProjectNode, DepConstraint]> | undefined {
  return externalDependencies
    .filter((dependency) =>
...
function packageExistsInPackageJson(packageName: string, projectRoot: string): boolean {
  const content = readFileIfExisting(join(workspaceRoot, projectRoot, 'package.json'))
  if (content) {
    const { dependencies, devDependencies, peerDependencies } = parseJson(content)
...
export function hasBuildExecutor(
  ...
  return !!(
    ...
    buildTargets.some(
      (target) =>
        projectGraph.data.targets[target] && projectGraph.data.targets[target].executor !== '',
    )
  )
}
 ...
 * @returns
 */
export function groupImports(importsToRemap: { member: string; importPath: string }[]): string {
  const importsToRemapGrouped = importsToRemap.reduce((acc, curr) => {
    ...
    return acc
  }, [] as { importPath: string; member: string }[])
...
export function appIsMFERemote(project: ProjectGraphProjectNode): boolean {
  const mfeConfig =
    readFileIfExisting(
      joinPathFragments(workspaceRoot, project.data.root, 'module-federation.config.js'),
    ) ||
    readFileIfExisting(
      joinPathFragments(workspaceRoot, project.data.root, 'module-federation.config.ts'),
    )
