...
import { getRootTsConfigFileName } from 'nx/src/utils/typescript'
import { defaultHashing, HashingImpl } from 'nx/src/hasher/hashing-impl'
import {
  FileData,
  ProjectGraph,
...
type ExpandedSelfInput = { fileset: string } | { runtime: string } | { env: string }
...
export class Hasher {
  ...
  constructor(
    ...
    const legacyRuntimeInputs = (
      this.options && this.options.runtimeCacheInputs ? this.options.runtimeCacheInputs : []
    ).map((r) => ({ runtime: r }))
...
class TaskHasher {
  ...
  private async hashSelfAndDepsInputs(
    ...
    const deps = await this.hashDepsInputs(depsInputs, projectGraphDeps, visited)
    ...
    const value = this.hashing.hashArray([...self.map((d) => d.value), ...deps.map((d) => d.value)])
          ...
          return await Promise.all(
            projectGraphDeps.map(async (d) => {
              if (visited.indexOf(d.target) > -1) {
                ...
                return await this.hashNamedInput(d.target, input.input || 'default', visited)
              }
            }),
          )
        }),
  ...
  private hashTarget(projectName: string, targetName: string): PartialHash {
    ...
    if (target.executor.startsWith(`@nrwl/`) && target.executor !== `@nx/workspace:run-commands`) {
    ...
    const hash = this.hashing.hashArray([JSON.stringify(this.projectGraph.externalNodes)])
    return {
      value: hash,
      details: {
  ...
  private async hashSelfInputs(
    ...
    for (let f of filesets) {
      ...
      } else if (f.startsWith('{workspaceRoot}/') || f.startsWith('!{workspaceRoot}/')) {
    ...
    return Promise.all([
      ...
      ...[...workspaceFilesets, ...this.legacyFilesetInputs.map((r) => r.fileset)].map((fileset) =>
        this.hashRootFileset(fileset),
      ),
...
export function getNamedInputs(nxJson: NxJsonConfiguration, project: ProjectGraphProjectNode) {
  return {
    default: [{ fileset: '{projectRoot}/**/*' }],
    ...nxJson.namedInputs,
...
export function extractPatternsFromFileSets(inputs: readonly ExpandedSelfInput[]): string[] {
  return inputs.filter((c): c is { fileset: string } => !!c['fileset']).map((c) => c['fileset'])
}
...
function expandSelfInputs(
  ...
  for (const d of inputs) {
    if (typeof d === 'string') {
      if (d.startsWith('^')) throw new Error(`namedInputs definitions cannot start with ^`)
...
export function filterUsingGlobPatterns(
  ...
  return files.filter((f) => {
    ...
    if (positive.length === 0 || (positive.length === 1 && positive[0] === `${root}/**/*`)) {
