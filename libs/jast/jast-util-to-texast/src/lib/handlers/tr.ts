...
import type { Tr } from "jast-types";
import type { AlignmentTab, Command, Linebreak, Text } from "texast";
import { all } from "../all.js";
import { type J, Node } from "../types.js";
...
export function tr(j: J, node: Tr) {
	...
	// @ts-expect-error: linebreak is not a valid texast node, but who care
	contents.push({ type: "linebreak", value: "\\", children: [] } as Linebreak);
	if (
		node.children.every(
			(child) =>
				child.type === "element" &&
		...
		const commandName = j.booktabs
			...
			: "hline";
		contents.push(
			// @ts-expect-error: type text is not assignable to alignmenttab? ???
			...[
